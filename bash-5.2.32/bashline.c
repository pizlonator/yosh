...
#  endif
  ...
  /* Enable LLM "yo" features for yosh */
  {
    static const char *yo_documentation =
      ...
      "The key feature is the 'yo' command: type `yo <natural language>` and the shell calls Claude\n"
      ...
      "## API Key Setup\n"
      "\n"
      "Yosh requires an Anthropic API key stored in `~/.yoshkey`.\n"
      "\n"
      "1. Create the file: `echo 'sk-ant-...' > ~/.yoshkey`\n"
      "2. Set permissions: `chmod 600 ~/.yoshkey`\n"
      "\n"
      "The file MUST have mode 0600 (readable only by owner). Yosh reads the key fresh\n"
      "on each API call, so you can update it without restarting the shell.\n"
      "\n"
      ...
      "to change keys, for example. But otherwise it's likely redundant.\n"
      "\n"
      ...
      "  When enabled (default), Claude can search the web and fetch web pages to\n"
      "  answer questions with up-to-date information. This uses the Anthropic API's\n"
      "  built-in web search ($10/1000 searches) and web fetch (no extra cost) tools.\n"
      "  Can be changed mid-session; takes effect on next yo command.\n"
      "\n"
      "## Response Types\n"
      "\n"
      ...
      "The API key file doesn't exist. Create it with your Anthropic API key.\n"
      "\n"
      "### '~/.yoshkey must have mode 0600'\n"
      "The file has wrong permissions. Run: `chmod 600 ~/.yoshkey`\n"
      "\n"
      "### '~/.yoshkey is empty'\n"
      "The file exists but contains no key. Add your API key to the file.\n"
      "\n"
      "### 'API error: ...' messages\n"
      "The Anthropic API returned an error. Common causes:\n"
      "- Invalid API key\n"
      "- Rate limiting\n"
      "- Model not available\n"
      "\n"
