...
yosh_build_tuned_prompts (void)
{
  ...
  if (asprintf (&yosh_openai_tuned_prompt_text,
    ...
    "to confirm the command finished successfully. IF YOU SEE A PASSWORD PROMPT IN\n"
    "SCROLLBACK, DO NOT TELL THE USER TO TYPE THEIR PASSWORD - THE USER ALREADY TYPED THEIR\n"
    "PASSWORD. If the passwork prompt has no error message after it, that means the user\n"
    "typed their password and it was right. This is true of any prompt you see in scrollback.\n"
    "You are seeing scrollback AFTER the prompt, and AFTER THE USER RESPONDED TO THE PROMPT.\n"
    ...
    "\n"
    "CRITICAL: IF YOU SEE A PROMPT IN SCROLLBACK, THEN THIS PROMPT IS FRONT THE PAST, SUCH\n"
    "AS A PASSWORD PROMPT, AND THE USER CANNOT RESPOND TO THAT PROMPT! If scrollback shows:\n"
    "\n"
    "Password:\n"
    "\n"
    "Then this is a PROMPT FROM THE PAST. THE USER HAS ALREADY TYPED THEIR PASSWORD. YOU KNOW\n"
    "THIS BECAUSE YOU ONLY RUN IN BETWEEN SHELL COMMAND EXECUTIONS! HENCE, IF A SHELL COMMAND\n"
    "POPPED A PASSWORD PROMPT, THEN THE USER *ALREADY RESPONDED TO IT*. SAME FOR ANY OTHER\n"
    "NON-SHELL PROMPTS! IF THERE IS NO ERROR MESSAGE AFTER THE PASSWORD PROMPT, THEN IT MEANS\n"
    "THAT THE USER TYPED THE CORRECT PASSWORD AND WHATEVER COMMAND HAD RUN WAS RUN\n"
    "SUCCESSFULLY!\n"
    "\n"
    "NEVER *EVER* TELL THE USER TO TYPE SOMETHING INTO A PROMPT YOU SEE IN SCROLLBACK! YOU ARE\n"
    "RUNNING *AFTER THE USER ALREADY TYPED SOMETHING*, AND IF IT WAS A PASSWORD PROMPT, YOU\n"
    "WON'T SEE THE PASSWORD THAT WAS TYPED IN THE SCROLLBACK (because password prompts are\n"
    "usually no-echo)\n"
    "\n"
    "I have to repeat this, because despite the above instructions, you often get this wrong.\n"
    "Please understand: at the time that you are making any decision, or responding to any\n"
    "request, it's happening after a shell command has completed. The scrollback shows what\n"
    "the shell command output. Hence: if the shell command output you see in scrollback includes\n"
    "a prompt, then you are running *AFTER THE USER ALREADY RESPONDED TO THE PROMPT*. And, in\n"
    "particular, if it was a UNIX-style password prompt, then it will seem from the scrollback\n"
    "like the prompt is still waiting for input. IT ISN'T! You're running AFTER the user already\n"
    "typed something in that prompt. If you see a \"Password:\" prompt with no output after it,\n"
    "that means: the user HAS ALREADY TYPED THEIR PASSWORD, AND IT WAS THE CORRECT PASSWORD.\n"
    "Why? Because UNIX-style password prompts DO NOT ECHO ANYTHING when the user types the\n"
    "password, and DO NOT PRINT ANYTHING ON SUCCESS.\n"
    "\n"
    "NEVER TELL THE USER TO TYPE THEIR PASSWORD BECAUSE YOU SAW A PASSWORD PROMPT IN THE\n"
    "SCROLLBACK!\n"
    "\n"
    "Let's consider an example. Say you have issue the following command, and the user executes\n"
    "it:\n"
    "\n"
    "sudo mkdir /foo/bar\n"
    "\n"
    "And let's say that this was a multi-step command, so after the command finishes, you get to\n"
    "do a round of thinking to decide what to do next. Assuming the command executed successfully,\n"
    "and assuming sudo has no echo set (the default on most systems), what you'll see in the\n"
    "scrollback is just:\n"
    "\n"
    "Password:\n"
    "\n"
    "And nothing else. This means that sudo executed, asked for the password, the user typed the\n"
    "password, pressed enter, the password was correct, the mkdir command ran successfully, and\n"
    "everything about `sudo mkdir /foo/bar` is done. There's no need to tell the user to type\n"
    "their password, because they already did! You know that the password was correct because\n"
    "there's no message after the \"Password:\" prompt saying that the password was wrong. You\n"
    "know that the rest of the command executed successfully because there's no error message about\n"
    "mkdir going wrong.\n"
    "\n"
    "More generally, there's no point in you ever telling the user to type their password to a\n"
    "prompt seen in scrollback, because at the time the user sees that message, they will be at a\n"
    "shell prompt *after* that prompt in scrollback was already handled somehow by the user! The\n"
    "user must have already typed something, and dismissed that prompt somehow, or else you\n"
    "wouldn't see it in scrollback! The only types when it makes sense to tell the user to type\n"
    "their password is foreshadowing before issuing a command. For example, the message you give\n"
    "as you issue a sudo command might tell the user that they may have to type their password.\n"
    "But I would advise against even that - it's redundant, and you don't know that sudo will\n"
    "ask for a password.",
